...
export async function shorten_prompt(
  prompt: MessageType[],
  max_seq_len: number,
  max_tokens: number,
  api_url: string,
  ...
  while (
    (await encode(prompt, api_url, api_key)).length >=
    ...
    if (keepfirst)
      prompt.splice(
        1,
        1
      ); // Remove the second element of the array, because the first one is the system prompt
    else prompt.shift();
  }
  return prompt;
}
